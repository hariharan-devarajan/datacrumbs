...
for ts in 256k; do # 4k 16k 64k 256k 1m 4m 16m 64m
  configuration="-o=${DATA_DIR}/test.bat-${ts} -F -m -b=1g -i 10 -d 10 -t=${ts}"
...
for ts in 256k; do # 4k 16k 64k 256k 1m 4m 16m 64m
  configuration="-o=${DATA_DIR}/test.bat-${ts} -F -m -b=1g -i 10 -d 10 -t=${ts} -O useO_DIRECT=1"
  echo "Running ${IOR_INSTALL_DIR}/bin/ior ${configuration} -w -k"
  LD_PRELOAD=$DATACRUMBS_SO ${IOR_INSTALL_DIR}/bin/ior ${configuration} -w -k
  sleep 10
  if [ "$DROP_CACHES" -eq "1" ];
...
for ts in 256k; do # 4k 16k 64k 256k 1m 4m 16m 64m
  configuration="-o=${DATA_DIR}/test.bat-${ts} -F -m -b=1g -i 10 -d 10 -t=${ts} -e -Y"
  echo "Running ${IOR_INSTALL_DIR}/bin/ior ${configuration} -w -k"
  LD_PRELOAD=$DATACRUMBS_SO ${IOR_INSTALL_DIR}/bin/ior ${configuration} -w -k
  sleep 10
  if [ "$DROP_CACHES" -eq "1" ];
...
for ts in 256k; do # 4k 16k 64k 256k 1m 4m 16m 64m
  configuration="-o=${DATA_DIR}/test.bat-${ts} -F -m -b=1g -i 10 -d 10 -t=${ts} -P"
  echo "Running ${IOR_INSTALL_DIR}/bin/ior ${configuration} -w -k"
  LD_PRELOAD=$DATACRUMBS_SO ${IOR_INSTALL_DIR}/bin/ior ${configuration} -w -k
  sleep 10
  if [ "$DROP_CACHES" -eq "1" ];
...
for DROP_CACHES in 0 1; do
  for ts in 256k; do # 4k 16k 64k 256k 1m 4m 16m 64m
    configuration="-o=${DATA_DIR}/test.bat-${ts} -F -m -b=1g -i 10 -d 10 -t=${ts} -a MPIIO"
  echo "Running ${IOR_INSTALL_DIR}/bin/ior ${configuration} -w -k"
    LD_PRELOAD=$DATACRUMBS_SO ${IOR_INSTALL_DIR}/bin/ior ${configuration} -w -k
    sleep 10
    if [ "$DROP_CACHES" -eq "1" ];
...
for DROP_CACHES in 0 1; do
  for ts in 256k; do # 4k 16k 64k 256k 1m 4m 16m 64m
    configuration="-o=${DATA_DIR}/test.bat-${ts} -F -m -b=1g -i 10 -d 10 -t=${ts} -a MPIIO -c"
  echo "Running ${IOR_INSTALL_DIR}/bin/ior ${configuration} -w -k"
    LD_PRELOAD=$DATACRUMBS_SO ${IOR_INSTALL_DIR}/bin/ior ${configuration} -w -k
    sleep 10
    if [ "$DROP_CACHES" -eq "1" ];
